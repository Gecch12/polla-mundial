...
document.getElementById('emailMeta').innerHTML=`
  ...
  ${prizes.map(p=>`<div class="prize-row has-tip" tabindex="0"><div class="prize-pos">#${p.pos}</div><div class="prize-name">${esc(p.row?.Participante||'—')}</div><div class="prize-points">${p.row?.Puntos??'—'} pts</div><div class="prize-amount">${esc(p.amount||'—')}</div></div>`).join('')}
  </div></div>`;
